...
pub struct ChunkManager {
    ...
    /// Queue of chunk positions waiting to be generated, ordered
    /// nearest-first so the player sees terrain appear close-up first.
    pub pending:     VecDeque<IVec3>,
    /// Mirror set for O(1) membership tests on `pending`.
    pub pending_set: std::collections::HashSet<IVec3>,
